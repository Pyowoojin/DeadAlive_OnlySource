...
{
	if(SkeletalComponent && SphereCollision && NoticeSphereCollision)
	{
		this->Owner = nullptr;
...
// BaseItemClass에서 소리를 재생 후 장착된 무기가 없다면 장착시킴
...
void ABaseWeapon::GunFire(const FHitResult &HitResult, AActor* Player)
{
	...
	if(WeaponHitResult.bBlockingHit && WeaponHitResult.ImpactPoint != HitResult.ImpactPoint)
	{
		...
		// UE_LOG(LogTemp, Error, TEXT("물체가 있음. 그 물체의 이름은 : %s"), *WeaponHitResult.GetComponent()->GetName());
	}
	else
	{
